...
namespace {
    class CPearlAddressVisitor : public boost::static_visitor<bool> {
    private:
        CPearlAddress *addr;
    public:
        CPearlAddressVisitor(CPearlAddress *addrIn) : addr(addrIn) { }
...
bool CPearlAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CPearlAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CPearlAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CPearlAddressVisitor(this), dest);
}

bool CPearlAddress::IsValid() const {
...
CTxDestination CPearlAddress::Get() const {
    if (!IsValid())
...
bool CPearlAddress::GetKeyID(CKeyID &keyID) const {
...
bool CPearlAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CPearlSecret::SetKey(const CKey& vchSecret) {
...
CKey CPearlSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CPearlSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CPearlSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CPearlSecret::SetString(const std::string& strSecret) {
