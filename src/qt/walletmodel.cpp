...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CPearlAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CPearlAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject pearl fee
        unsigned int pearlfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        pearlfee = ((MIN_RELAY_TX_FEE * pearlfee) * 10000);
        if (nFeeRequired > pearlfee){
            LogPrintf("nFeeRequired: %d -- PearlaprojectFee: %d\n", nFeeRequired, pearlfee);
            return PearlaprojectFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CPearlAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CPearlAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CPearlAddress(address).ToString());
...
{
    // Pearl context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CPearlAddress(address).ToString())].push_back(out);
    }
}
