...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePearlAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePearlAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePearlAmount = strAnonymizePearlAmount.remove(strAnonymizePearlAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePearlAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePearlAmount*COIN) nMaxToAnonymize = nAnonymizePearlAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePearlAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePearlAmount));
        strAnonymizePearlAmount = strAnonymizePearlAmount.remove(strAnonymizePearlAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePearlAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
