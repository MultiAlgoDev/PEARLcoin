...
//
// Bootup the masternode, look for a 50,000 PEARL input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in Pearl.conf */
...
{
    CPearlAddress address(collateralAddress);
