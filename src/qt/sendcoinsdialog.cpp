    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Pearl address (e.g. iACLambeGgkxodKtfzM7huFXyPYWjz33Z2)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CPearlAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to 500 PEARL.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
    ...
    case WalletModel::NarrationTooLong:
        ...
        break;
    case WalletModel::PearlaprojectFee:
        msgParams.first = tr("A fee %1 times higher than %2 per recipient is considered an pearlly high fee.").arg(10000).arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MIN_RELAY_TX_FEE));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CPearlAddress addr = CPearlAddress(text.toStdString());
