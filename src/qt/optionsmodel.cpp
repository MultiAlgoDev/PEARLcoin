...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePearlAmount"))
        settings.setValue("nAnonymizePearlAmount", 1000);
    nAnonymizePearlAmount = settings.value("nAnonymizePearlAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePearlAmount"))
        SoftSetArg("-anonymizePearlamount", settings.value("nAnonymizePearlAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePearlAmount:
            return QVariant(nAnonymizePearlAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePearlAmount:
            nAnonymizePearlAmount = value.toInt();
            settings.setValue("nAnonymizePearlAmount", nAnonymizePearlAmount);
            emit AnonymizePearlAmountChanged(nAnonymizePearlAmount);
            break;
        default:
            break;
        }
