...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Pearl.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Pearld.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 2222)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePearlamount=<n> " + _("Keep N Pearl anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pearl is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pearl is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Pearl version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizePearlAmount = GetArg("-anonymizePearlamount", 0);
    if(nAnonymizePearlAmount > 999999) nAnonymizePearlAmount = 999999;
    if(nAnonymizePearlAmount < 2) nAnonymizePearlAmount = 2;
    ...
    LogPrintf("Anonymize Pearl Amount %d\n", nAnonymizePearlAmount);
